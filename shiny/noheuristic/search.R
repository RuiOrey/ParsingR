...
if(!exists('m_heuristics'))
	load('m_heuristics.RData')
if(!exists('m'))
	load('m.RData')
...
search_heuristics <- function(keywords, max=100,scalesize=c(4,.5))
{
	keywords <- transform_text(keywords)
	w <- rowSums(as.matrix(m_heuristics[,keywords]))

	library(wordcloud)
	wordcloud(names(w), w, max.words=max, colors=brewer.pal(8,"Dark2"),scale=scalesize)
}

search_heuristics_documentSize <- function(keywords, max=100,scalesize=c(4,.5))
{
	keywords <- transform_text(keywords)
	w <- rowSums(as.matrix(m_heuristics[,keywords]))
