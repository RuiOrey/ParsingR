...
function findWord($word) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    */
    ...
    $temp_database[$row['name']] = array($row['weight'], $row['times'], $row['abstract']);
  }
...
<?php
  if ($ok == true) {
    ...
    echo "<div style='font-family: arial, sans-serif;font-size: 13px;'>Best places in the world for '".$_POST['keywords']."' - About ".count($database)." results.</div>";
    ...
    foreach($preserved as $key => $item) {
      ...
      echo "<div><h2 style='margin-bottom:0px;'><a href='https://en.wikivoyage.org/wiki/".$key."' target='_blank'>".$key."</a> - <span style='font-size:16px;'>".round($item,2)."</span></h2></div>";
